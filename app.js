...
function setupEditFormListeners(id) {
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      const index = AppState.inspections.findIndex(i => i.id === id);
      if (index !== -1) {
        // Merge existing record with updated data — preserved photos survive
        AppState.inspections[index] = { ...AppState.inspections[index], ...data, updatedAt: new Date().toISOString() };
        AppState.save();
